...
class MichaelJordan(QCAlgorithm):

    def Initialize(self):
        self.SetStartDate(2018, 1, 1)  # Set Start Date
        self.SetEndDate(2021, 1, 1)
        self.SetCash(100000)  # Set Strategy Cash
        self.qqq = self.AddEquity("GOOGL", Resolution.Hour).Symbol
    ...
    def OnOrderEvent(self, orderEvent):
        ...
        if self.entryTicket is not None and self.entryTicket.OrderId == orderEvent.OrderId:
            self.stopMarketTicket = self.StopMarketOrder(self.qqq, -self.entryTicket.Quantity, 0.9*self.entryTicket.AverageFillPrice)
